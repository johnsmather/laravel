...
start.on("click", () => {
    if(minimal_char.val() > max_char.val()) {
        ...
        let i = 1
        let num = getRandomInt(Number(min), Number(max))
        task.text("$$" + num + "^{2}$$")
        MathJax.typeset()
        let task_ans = num**2
        function check(min, max) {
            ...
            if(i === 11) {
                $.ajaxSetup({
                    ...
                    let points = Math.floor(Math.floor(10000 / time) * correct_answers * (Math.abs(min) + Math.abs(max)) * 0.0003 * (Math.abs(max) - Math.abs(min)))
